...
test "put get delete and recover from wal" {
    inline for ([_]db_mod.Backend{ .rwlock, .cow }) |backend| {
        ...
        {
            const db = try db_mod.KvDb.openInDir(allocator, tmp.dir, "test.wal", backend);
            defer db.close();
        ...
        {
            const reopened = try db_mod.KvDb.openInDir(allocator, tmp.dir, "test.wal", backend);
...
test "concurrent readers see whole values while one writer updates" {
    inline for ([_]db_mod.Backend{ .rwlock, .cow }) |backend| {
        ...
        const db = try db_mod.KvDb.openInDir(allocator, tmp.dir, "concurrent.wal", backend);
        defer db.close();
